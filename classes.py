...
class Init():
    ...
    def __init__(self, model_name, data_path, real_file, train_all_ratio=1/3, n_steps=5, 
        ...
        self.is_nn = 'MLP' in self.model_name or 'LSTM' in self.model_name or 'TCN' in self.model_name
#         print (f'is a network: {self.is_nn}')
    ...
    def prepare_data(self):
        ...
        if 'MLP' in self.model_name or not self.is_nn:
            # flatten input
    ...
    def create_model(self):
        if 'MLP' in self.model_name:
            ...
            self.model = RandomForestRegressor(n_estimators=self.n_neurons, 
                                               max_depth=self.max_depth, bootstrap=True, n_jobs=-1)

        elif 'SV' in self.model_name:
            self.model = MultiOutputRegressor(SVR(kernel='poly', C=100, gamma='auto', degree=3, epsilon=.1, coef0=1))
            
        elif 'LGB' in self.model_name:
            self.model = MultiOutputRegressor(LGBMRegressor(n_estimators=self.n_neurons, 
                                       max_depth=self.max_depth, reg_alpha=self.reg_alpha))
            
        elif 'AB' in self.model_name:
            self.model = MultiOutputRegressor(AdaBoostRegressor(n_estimators=self.n_neurons))    
    ...
    def predict(self, plot_losses=True, plot_preds=True, plot_errs=False, print_res=False):
        ...
        data = {}
        data["model_name"] = f'{self.model_name}{self.prefix}'
        if self.is_nn:
    ...
    def save(self):
        if not self.is_nn:
            pickle.dump(self.model, open(f'{self.data_path}/{self.model_name}{self.prefix}.hdf', "wb"))
        elif 'TCN' in self.model_name:
            # get model as json string and save to file
            model_as_json = self.model.to_json()
            with open(f'{self.data_path}/{self.model_name}{self.prefix}.json', "w") as json_file:
                json_file.write(model_as_json)
            # save weights to file (for this format, need h5py installed)
            self.model.save_weights(f'{self.data_path}/{self.model_name}{self.prefix}.h5')
        else:
            self.model.save(f'{self.data_path}/{self.model_name}{self.prefix}')#, save_format='h5')

        print (f'{self.data_path}/{self.model_name}{self.prefix}.hdf saved')
...
#             print (self.X_reshaped.shape[0], self.X_val_reshaped.shape[0])
            self.model_inf = self.model.fit(self.X_reshaped[:shape_train], self.y[:shape_train], 
                                       epochs=self.n_epochs, verbose=verbose, 
                                       validation_data=(self.X_val_reshaped[:shape_test], self.y_val[:shape_test]))
        elif not self.is_nn:
...
class Gen():
    ...
    def __init__(self, model_name, study_path, real_file, gen_dfs_path, tpl_path, train_all_ratio=1/3, n_steps=5, 
        ...
        self.X_all_reshaped_s, self.raw_seq1_norm_s, self.true_data_norm_s, self.true_data_s = [], [], [], []
        self.scalers = []
        self.train_lens = []
        self.inputss, self.outputss = [], []
...
#         print (f'{len(self.inputss)} out of {len(tpl_files)} ({np.round(len(self.inputss)/len(tpl_files)*100, 1)}%) without errors total')
    ...
    def create_model(self):
        ...
        if 'MLP' in self.model_name:
            ...
            self.model = RandomForestRegressor(n_estimators=1000, max_depth=10, bootstrap=True, n_jobs=-1)
...
#         self.prefix = '-tuning' if self.tuning else '-transf'
        ...
        if not self.is_nn:
            ...
            self.model.save(f'{self.study_path}/{self.model_name}{self.prefix}.hdf')#, save_format='h5')
